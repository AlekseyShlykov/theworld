 ...
 */
export function sendChoiceEvent(round: number, choice: number, language?: Language | string): void {
  const url = import.meta.env.VITE_ANALYTICS_WEB_APP_URL;
  if (!url || typeof url !== 'string' || url.trim() === '') {
    if (!_analyticsUrlWarned) {
      _analyticsUrlWarned = true;
      console.warn(
        '[Analytics] VITE_ANALYTICS_WEB_APP_URL is not set. Choice events will not be sent. ' +
        'For GitHub Pages: add repository secret VITE_ANALYTICS_WEB_APP_URL and re-run the workflow.'
      );
    }
    return;
  }
  ...
  if (import.meta.env.DEV) {
    console.debug('[Analytics] Sending event:', { round, choice, language: lang, sessionId: payload.sessionId });
  }
